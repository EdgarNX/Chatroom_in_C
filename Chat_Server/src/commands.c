...
void cmd_login(client_t* client, client_t** clients)
{
	...
	if (login && passwd){
		if(!(result = sign_in(login, passwd, client, clients))){
			...
			sprintf(buff_out, "[SERVER]: %s has logged in.\n", client->name);
			send_msg(buff_out, client->uid, clients);
		}
		else if(result==1){
			send_private_msg("[SERVER]: Incorrect password. Please, try again.\n", client->uid, clients);
		}
		else if(result==2){
			send_private_msg("[SERVER]: User is already online.\n", client->uid, clients);
		}
		else{
			send_private_msg("[SERVER]: User not found. Please, try again.\n", client->uid, clients);
		}
	}
	else{
		send_private_msg("[SERVER]: Use \\login [login] [passwd]\n", client->uid, clients);
	}
}
...
void cmd_register(client_t* client, client_t** clients)
{
	...
	if (login && passwd){
		if(!(result = sign_up(login, passwd, client))){
			char buff_out[BUFFER_SIZE];
			sprintf(buff_out, "[SERVER]: %s has registered.\n", client->name);
			send_msg(buff_out, client->uid, clients);
		}
		else if(result==1){
			send_private_msg("[SERVER]: User already exists. Please, try again!\n", client->uid, clients);
		}
	}
	else{
		send_private_msg("[SERVER]: Use \\register [login] [passwd]\n", client->uid, clients);
	}
}
...
{
	char buff_out[BUFFER_SIZE];
	sprintf(buff_out, "[SERVER]: %s has left the server.\n", client->name);
	send_msg(buff_out, client->uid, clients);
}
...
void cmd_pm(client_t* client, client_t** clients)
{
	...
	if (!name || !(buff=strtok(NULL, "\0"))){
		send_private_msg("[SERVER]: Use \\pm [user] [msg]\n", client->uid, clients);
	}
	else{
		if((uid = get_user_id(name, clients))){
			...
			sprintf(buff_out, "[SERVER]: %s is offline.\n", name);
...
void cmd_name(client_t* client, client_t** clients)
{
	...
	if(param){
		...
		if(!rename(filepath, newfilepath)){
			...
			sprintf(buff_out, "[SERVER]: %s is now %s.\n", old_name, client->name);
			send_msg_all(buff_out, clients);
		}
		else{
			send_private_msg("[SERVER]: Can't rename.\n", client->uid, clients);
		}
	}else{
		send_private_msg("[SERVER]: Incorrect name. Please, try again.\n", client->uid, clients);
	}
}
...
void cmd_passwd(client_t* client, client_t** clients)
{
	...
	if(old_passwd && new_passwd){
		...
		if (file==NULL){
			send_private_msg("[SERVER]: Can't change password.\n", client->uid, clients);
		}
		else{
			...
			if(!strcmp(buff,old_passwd)){
				...
				send_private_msg("[SERVER]: Password changed.\n", client->uid, clients);
			}
			else{
				send_private_msg("[SERVER]: Incorrect password. Please, try again.\n", client->uid, clients);
			}
		...
		send_private_msg("[SERVER]: Use \\passwd [old] [new]\n", client->uid, clients);
	}
}
...
void cmd_help(client_t* client, client_t** clients)
{
	...
	strcat(buff_out, "\\q|(quit)|(exit)Leave chat;\n");
	strcat(buff_out, "-------------------------------------\n");
	send_private_msg(buff_out, client->uid, clients);
}
...
{
	char buff_out[BUFFER_SIZE];
	sprintf(buff_out, "[SERVER]: %s is unknown command. Type \\help for help.\n", command);
	send_private_msg(buff_out, client->uid, clients);
}

	...
	while((read(client->clientfd,buff_in,BUFFER_SIZE-1))>0)
	{
		...
		if(!strlen(buff_in))
		{
			...
			else if(!strcmp(command,"\\register"))
			{
				...
				send_private_msg("[SERVER]: You are not authorized! (type \\login or \\register)\n", client->uid, clients);
			}
		}
		...
		{
			sprintf(buff_out, "[%s]: %s\n", client->name, buff_in); //writes formatted data to string (buff_out)
			printf("[SYSTEM]: %s", buff_out);
